...
public static class DbContextExtensions
{
    ...
    ///
    /// Supported relationship patterns: one-to-many, pure many-to-many,
    /// many-to-many with payload, required one-to-one, optional one-to-one.
    ///
    /// Unsupported patterns (many-to-one references) in loaded navigations are
    /// silently skipped if unchanged, or rejected if mutations are detected
    /// (FR-018/FR-019).
    ///
    ...
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="context"/>, <paramref name="existingEntity"/>, or <paramref name="updatedEntity"/> is null.</exception>
        ...
        where T : class
    {
        ...
        return GraphUpdateOrchestrator.UpdateGraph(context, existingEntity, updatedEntity);
    }
}
